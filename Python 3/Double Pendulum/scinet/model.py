...
class Network(object):
    ...
    # The first function asks for the size of the input, the amount of neurons, the size of the second input into the decoder, and the output size.
    # I will try to understand what each of these four values do in the model. 
    
    # The first one, the input size, is the length of the data vector. In our case, we have that a data vector has 50 equally spaced values, which
    # means that this value should be 50.
    
    # The latent size determines the amount of layers that are necessary for the data to be trained. In our case, we need one for b and one for 
    # kappa. 
    
    # The output size is exactly the same as the input size, but for the x_out vector. Here it is 1, because the vector is reshaped during generation.

    def __init__(self, input_1_size, input_2_size , latent_size, time_size, output_1_size, output_2_size,
                 encoder_num_units=[100, 100], decoder_num_units=[100, 100], name='Unnamed',
                 tot_epochs=0, load_file=None):
        """
        Parameters:
        input_1_size: length of a single data vector.
        input_2_size: length of a single data vector.
        latent_size: number of latent neurons to be used.
        time_size: number of neurons for 2nd input into decoder.
        output_1_size: length of a single label vector.
        output_2_size: length of a single label vector.
        ...
        # We define the whole bunch of necessary variables.
        ...
        self.output_1_size = output_1_size
        self.output_2_size = output_2_size
        ...
        with self.graph.as_default():
            initialize_uninitialized(self.session) # Initialize all parameters.
        ...
        with self.graph.as_default():
            ...
            for epoch_iter in tqdm_notebook(list(range(epoch_num))):
                ...
                for step, data_dict in enumerate(self.gen_batch(training_data, batch_size)):
                    ...
                    feed_dict = {**data_dict, **parameter_dict}
    ...
    def save(self, file_name):
        ...
        with self.graph.as_default():
            ...
            params = {'latent_size': self.latent_size,
                      'input_1_size': self.input_1_size,
                      'input_2_size': self.input_2_size,
                      'time_size': self.time_size,
                      'output_1_size': self.output_1_size,
                      'output_2_size': self.output_2_size,
    ...
    def graph_setup(self):
        ...
        with self.graph.as_default():
            ...
            # Define placeholders #
            #######################
            self.input_1 = tf.placeholder(tf.float32, [None, self.input_1_size], name='input1')
            self.input_2 = tf.placeholder(tf.float32, [None, self.input_2_size], name='input2')
            ...
            self.time = tf.placeholder(tf.float32, shape=[None, self.time_size], name='time')
            self.labels_1 = tf.placeholder(tf.float32, shape=[None, self.output_1_size], name='labels1')
            self.labels_2 = tf.placeholder(tf.float32, shape=[None, self.output_2_size], name='labels2')
            ...
            with tf.variable_scope('encoder'):
                ...
                enc_in_dims = [self.input_1_size] + self.encoder_num_units
                enc_out_dims = self.encoder_num_units + [2 * self.latent_size]
                temp_layer = self.input_1
            ...
            with tf.name_scope('latent_layer'):
                ...
                self.mu_with_time = tf.concat([self.mu_sample, self.time], axis=1)

            with tf.name_scope('kl_loss'):
                self.kl_loss = kl_divergence(self.mu, self.log_sigma, dim=self.latent_size)

            with tf.variable_scope('decoder'):
                temp_layer = self.mu_with_time

                dec_in_dims = [self.latent_size + self.time_size] + self.decoder_num_units
                dec_out_dims = self.decoder_num_units + [self.output_1_size]
            ...
            with tf.name_scope('recon_loss'):
                self.recon_loss = tf.reduce_mean(tf.reduce_sum(tf.squared_difference(self.labels_1, self.output), axis=1))
    ...
    def gen_batch(self, data, batch_size, shuffle=True, random_epsilon=True):
        ...
        if shuffle:
            ...
            data = [data[i][p] for i in [0, 1, 2, 3, 4]]
        for i in range(epoch_size):
            batch_slice = slice(i * batch_size, (i + 1) * batch_size)
            batch = [data[j][batch_slice] for j in [0, 1, 2, 3, 4]]
    ...
    def gen_data_dict(self, data, random_epsilon=True):
        ...
        return {self.input_1: data[0],
                self.input_2: data[1],
                self.time: data[2],
                self.labels_1: data[3],
                self.labels_2: data[-1],
                self.epsilon: eps}
